...
### 並列起動パターン
1. **調査**: 「〇〇を調べる」が複数 → 各調査を別subagentに
2. **実装**: 複数ファイル編集 → ファイルごとにsubagent並列起動
3. **検証**: テスト・lint・型チェック → 並列実行
4. **レビュー**: 実装後 → code-reviewer/quality-fixer起動

### モデル選択の指針
タスクの複雑さに応じて最小コストのモデルを選択：

| モデル | 用途 |
|--------|------|
| **haiku** | 単純な検索・grep・ファイル読み取り・lint実行・フォーマット確認・定型的な置換 |
| **sonnet** | 中程度の実装・コードレビュー・テスト作成・リファクタリング・ドキュメント生成 |
| **opus** | 複雑な設計判断・アーキテクチャ決定・難解なバグ修正・セキュリティ分析 |

**原則**: 迷ったらsonnet。明らかに単純ならhaiku。本当に難しい時だけopus。

...
function checkContent(content, filePath) {
  ...
  lines.forEach((line, index) => {
    ...
    const timerStringMatch = line.match(
  ...
  lines.forEach((line, index) => {
    ...
    if (/dangerouslySetInnerHTML/.test(line)) {
      ...
      for (
        ...
        i < Math.min(lines.length, index + 5);
        i++
      ) {
  ...
  lines.forEach((line, index) => {
    ...
    // window系の直接操作
    const windowMethods =
      /window\.(scrollTo|scroll|alert|confirm|prompt|location\.href\s*=)/;
  ...
  lines.forEach((line, index) => {
    ...
    const arrayPattern =
      /(?:(?:array|list|items|elements|data|values|results|collection|arr)\w*|[a-z]+(?:Array|List|Items|Elements|Data|Values|Results|Collection))\.(push|pop|shift|unshift|splice|sort|reverse)\s*\(/i;

    // router.push, history.push, navigation.push などのルーティング関連は除外
    const routerPattern =
      /(?:router|route|history|navigation|navigate|nav|location)\.\w*\.?push\s*\(/i;

    // stack.push, queue.push などのデータ構造操作も除外
    const dataStructurePattern =
      /(?:stack|queue|heap|tree|graph)\.\w*\.?push\s*\(/i;

    const match = line.match(arrayPattern);

    if (
      match &&
      !routerPattern.test(line) &&
      !dataStructurePattern.test(line)
    ) {
      ...
      // 明確に配列を示す変数名パターン
      const isLikelyArray =
        /(?:array|list|items|elements|data|values|results|collection|^arr$)/i.test(
          variableName,
        ) ||
        /(?:Array|List|Items|Elements|Data|Values|Results|Collection)$/.test(
          variableName,
        );
  ...
  lines.forEach((line, index) => {
    ...
    // 制御構造の開始を検出
    const controlFlowMatch = trimmedLine.match(
      /\b(if|for|while|switch|try|catch|finally|else\s+if)\s*\(/,
    );
    ...
    if (controlFlowMatch || doMatch || elseMatch) {
      // 制御構造の種類を記録
      const controlType = controlFlowMatch
        ? controlFlowMatch[1]
        : doMatch
          ? "do"
          : "else";
      ...
      const prevControlFlow = prevLine.match(
        /\b(if|for|while|switch|try|catch|finally|else\s+if)\s*\(/,
      );
      ...
      if (prevControlFlow || prevDo || prevElse) {
        const controlType = prevControlFlow
          ? prevControlFlow[1]
          : prevDo
            ? "do"
            : "else";
  ...
  lines.forEach((line, index) => {
    ...
    if (
      /[a-zA-Z_$][a-zA-Z0-9_$]*!\./g.test(line) ||
      /[a-zA-Z_$][a-zA-Z0-9_$]*!\s*[;,\)\}]/g.test(line)
    ) {
      errors.push({
  ...
  lines.forEach((line, index) => {
    ...
    const typeSuffixMatch = line.match(
      /\b(type|interface)\s+(\w*(?:Type|Interface))\b/,
    );
    if (typeSuffixMatch) {
      errors.push({
        type: "type_suffix",
...
function printSummary(errors, warnings, filePath) {
  ...
  if (errors.length > 0) {
    ...
    console.error(
      `${colors.red}🚫 書き込みがブロックされました${colors.reset}`,
    );
    console.error(
      `${colors.red}════════════════════════════════════════════${colors.reset}`,
    );
    ...
    if (errorTypes.includes("fetch_internal_api_literal")) {
      ...
      console.error(
        `     例: const API_ENDPOINTS = { USERS: '/api/users' } as const;`,
      );
      console.error(
    ...
    if (
      ...
      errorTypes.includes("window_direct_manipulation") ||
      errorTypes.includes("style_direct_manipulation")
    ) {
      console.error(
    ...
    if (
      errorTypes.includes("inline_style_forbidden") ||
      errorTypes.includes("style_tag_forbidden")
    ) {
      console.error(
      ...
      console.error(
        `     CSS Modules, styled-components, emotion, Tailwind CSSなど`,
      );
      console.error(`     スタイルの分離で保守性を向上`);
    }
    if (
      errorTypes.includes("for_loop_forbidden") ||
      errorTypes.includes("foreach_forbidden")
    ) {
      console.error(
    ...
    if (
      errorTypes.includes("interface_i_prefix") ||
      errorTypes.includes("type_t_prefix") ||
      errorTypes.includes("type_suffix")
    ) {
      console.error(
    ...
    if (
      errorTypes.includes("loose_equality") ||
      errorTypes.includes("loose_inequality")
    ) {
      console.error(
    ...
    if (errorTypes.includes("non_null_assertion")) {
      ...
      console.error(
        `     array.find()! → const result = array.find(); if (result) {...}`,
      );
      console.error(`     オプショナルチェーン（?.）の使用も検討`);
    }
    if (
      errorTypes.includes("filename_underscore") ||
      errorTypes.includes("filename_camelcase") ||
      errorTypes.includes("filename_uppercase")
    ) {
      console.error(
      ...
      console.error(
        `     例外: Python/Goではsnake_caseが許可、React Component（.tsx/.jsx）は大文字許可`,
      );
      console.error(
        `     _から始まるファイルは許可（例: _app.tsx, _document.tsx）`,
      );
    }
    if (
      errorTypes.includes("number_in_component_name") ||
      errorTypes.includes("number_in_function_name")
    ) {
      console.error(
      ...
      console.error(
        `     説明的な名前を使用して、数字による区別を避けてください`,
      );
    }
    if (
      errorTypes.includes("eval_usage") ||
      errorTypes.includes("new_function_usage") ||
      errorTypes.includes("timer_string_usage")
    ) {
      console.error(
      ...
      console.error(
        `     new Function() → 事前定義した関数、ファクトリパターン`,
      );
      console.error(
    ...
    if (
      errorTypes.includes("ts_ignore_usage") ||
      errorTypes.includes("ts_nocheck_usage") ||
      errorTypes.includes("ts_expect_error_no_reason")
    ) {
      console.error(
    ...
    if (
      errorTypes.includes("loose_object_type") ||
      errorTypes.includes("loose_function_type") ||
      errorTypes.includes("any_array_type") ||
      errorTypes.includes("untyped_promise") ||
      errorTypes.includes("untyped_array")
    ) {
      console.error(
        `  27. ${colors.yellow}"型の緩いコード"${colors.reset} → 具体的な型定義を使用`,
      );
      console.error(
        `     Object → interface User { name: string; age: number; }`,
      );
      console.error(`     Function → (x: number, y: number) => number`);
      console.error(
        `     any[] → string[]、User[]、Array<{id: number; name: string}>`,
      );
      console.error(
        `     Promise → Promise<User>、Promise<void>、Promise<string[]>`,
      );
      console.error(
        `     Array → Array<string>、Array<User>、またはstring[]、User[]`,
      );
      console.error(
        `     型安全性を確保するため、常に具体的な型を使用してください`,
      );
    }
    if (
      errorTypes.includes("await_without_try_catch") ||
      errorTypes.includes("promise_without_catch") ||
      errorTypes.includes("empty_catch_block")
    ) {
      console.error(
      ...
      console.error(
        `     catch (e) {} → console.error(e) またはエラー処理を実装`,
      );
      console.error(
        `     例: try { await fetch() } catch (error) { handleError(error) }`,
      );
    }
    if (
      errorTypes.includes("localstorage_sensitive_data") ||
      errorTypes.includes("sessionstorage_sensitive_data")
    ) {
      console.error(
      ...
      console.error(
        `     機密情報（password, token, apiKey等）は絶対に保存しない`,
      );
    }
    if (errorTypes.includes("react_index_as_key")) {
      ...
      console.error(
        `     一意のIDがない場合: crypto.randomUUID() または nanoid`,
      );
    }
    if (errorTypes.includes("else_usage")) {
      ...
      console.error(
        `     コードの可読性とネストの削減のため早期リターンを使用してください`,
      );
    }
    if (errorTypes.includes("too_many_parameters")) {
      ...
      console.error(
        `     function create(name, age, email, address, phone) {...}`,
      );
      console.error(`     ↓`);
      console.error(
        `     function create({ name, age, email, address, phone }) {...}`,
      ...
      console.error(
        `     5個以上の引数は保守性を損なうため、オブジェクトでまとめてください`,
      );
    }
    if (errorTypes.includes("deep_control_nesting")) {
      ...
      console.error(
        `     if/for/while/switch/try-catchのネストは3層までに制限`,
      );
    ...
    if (errorTypes.includes("switch_statement_forbidden")) {
      ...
      console.error(
        `     switch文は禁止されています。ts-patternのmatch関数を使用してください`,
      );
      console.error(`     `);
    ...
    console.error("");
    console.error(
      `${colors.cyan}修正後、再度実行してください。${colors.reset}`,
    );

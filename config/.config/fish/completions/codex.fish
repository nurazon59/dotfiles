...
complete -c codex -n "__fish_codex_using_subcommand debug; and not __fish_seen_subcommand_from app-server help" -s c -l config -d 'Override a configuration value that would otherwise be loaded from `~/.codex/config.toml`. Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as TOML. If it fails to parse as TOML, the raw string is used as a literal' -r
complete -c codex -n "__fish_codex_using_subcommand debug; and not __fish_seen_subcommand_from app-server help" -l enable -d 'Enable a feature (repeatable). Equivalent to `-c features.<name>=true`' -r
complete -c codex -n "__fish_codex_using_subcommand debug; and not __fish_seen_subcommand_from app-server help" -l disable -d 'Disable a feature (repeatable). Equivalent to `-c features.<name>=false`' -r
complete -c codex -n "__fish_codex_using_subcommand debug; and not __fish_seen_subcommand_from app-server help" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c codex -n "__fish_codex_using_subcommand debug; and not __fish_seen_subcommand_from app-server help" -f -a "app-server" -d 'Tooling: helps debug the app server'
complete -c codex -n "__fish_codex_using_subcommand debug; and not __fish_seen_subcommand_from app-server help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'

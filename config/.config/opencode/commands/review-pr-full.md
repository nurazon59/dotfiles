---
description: 18 agentを3並列×6 Waveで実行し、各観点から徹底的にPRレビュー
agent: build
---
...
| # | Agent | 出力ファイル | 観点 |
|---|-------|-------------|------|
| 1 | general-purpose (code-reviewer) | `01-code-quality.md` | 規約、ベストプラクティス、可読性、命名 |
| 2 | general-purpose (code-explorer) | `02-codebase-impact.md` | 既存コードへの影響、実行パス、依存関係 |
| 3 | 言語特化agent（動的選択）| `03-language-specific.md` | 言語固有のイディオム、最新機能活用 |

> 言語特化: TypeScript→react-frontend-specialist / Python→general-purpose / Go→general-purpose / Rust→general-purpose
...
| # | Agent | 出力ファイル | 観点 |
|---|-------|-------------|------|
| 4 | general-purpose (code-simplifier) | `04-simplification.md` | 複雑なコードの簡素化、リファクタリング候補 |
| 5 | comment-cleaner | `05-comments.md` | コメントの正確性、長期保守性、技術的負債 |
| 6 | general-purpose (type-design) | `06-type-design.md` | 型設計、カプセル化、不変条件 |
...
| # | Agent | 出力ファイル | 観点 |
|---|-------|-------------|------|
| 7 | general-purpose (error-handling) | `07-error-handling.md` | サイレント失敗、握りつぶし、不適切なcatch |
| 8 | general-purpose (test-analyzer) | `08-test-coverage.md` | カバレッジ、エッジケース、テストの妥当性 |
...
| # | Agent | 出力ファイル | 観点 |
|---|-------|-------------|------|
| 10 | general-purpose (architect) | `10-architecture.md` | 設計パターン、責任分離、依存関係、スケーラビリティ |
...
| 14 | general-purpose (ui-design) | `14-ui-design.md` | レイアウト、スペーシング、視覚的階層 |
| 15 | general-purpose (accessibility) | `15-accessibility.md` | ARIA、キーボード、スクリーンリーダー、コントラスト |

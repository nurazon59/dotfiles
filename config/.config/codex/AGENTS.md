...
- 完了時は必ず検証（全て通るまで完了と言わない）:
  ...
  2. 変更の影響範囲に応じた確認を**全て**実行する（推測で省略しない）
     - UI/動作に影響 → Codexのブラウザ検証機能、または利用可能なブラウザ自動化手段で実動作確認
...
- **スクリプト配置**: 使い捨ては `zzz/`、再利用するなら skill 配下 (`~/.config/codex/skills/<skill>/scripts/`)、プロジェクト固有なら `scripts/`
...
- **並列調査を推奨**: 複数の調査項目がある場合は、research skillまたはsubagentを並列で活用する
- **検索コストを気にするな**: 検索・調査のコストより、誤った回答のコストを優先する
